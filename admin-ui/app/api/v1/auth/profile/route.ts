...
export async function GET(request: NextRequest) {
  ...
  if (!token) {
    console.warn("[API/v1/auth/profile/GET] Unauthorized - no token");
    return unauthorized();
  }

  const url = `${API_BASE_URL}/api/v1/auth/profile`;
  console.log(`[API/v1/auth/profile/GET] Fetching: ${url}`);
  ...
  try {
    ...
    console.log(`[API/v1/auth/profile/GET] Status: ${res.status}`, res.ok ? "✓ OK" : "✗ ERROR", data);
    ...
    console.error("[API/v1/auth/profile/GET] Network error:", err);
